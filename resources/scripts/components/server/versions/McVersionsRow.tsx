                        ...
                        const Download = setInterval(function () {
                            getVersionFileSize(uuid, filename)
                                .then((size) => {
                                    if (data !== size) {
                                        setPourcentage(
                                            `Download in progress ${bytesToString(size)}/${bytesToString(
                                                data
                                            )} (${bytesToString(size - oldsize)}/s)`
                                        );
                                        oldsize = size;
                                    } else {
        ...
        <GreyRowBox css={tw`grid grid-rows-2`} className={className}>
            ...
            >
                <p css={tw`text-neutral-300`}>
                    This action remove server.jar,libraries,mods,resources,scripts,fontfiles,oresources and config
            ...
            <div css={tw`mx-auto`}>
                <div css={tw`flex`}>
                    <img
                        src={
                            stype !== 'modpacks'
                                ? `https://cdn.bagou450.com/img/${stype}-icon.jpg`
                                : modpacktype !== 'others'
                                ? `https://cdn.bagou450.com/img/modpacks/${minecraftVersions.name.toLowerCase()}.png`

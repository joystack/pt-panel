...
const VersionSelect = () => {
    const { versionsType, setVersionsType, setPage, modpacktype, setModpacktype } =
        useContext(MinecraftVersionsContext);
    return (
        <>
            <p>
            ...
            <Select
                onChange={(e) => {
                    ...
                    setVersionsType(e.target.value);
                    setPage(1);
                }}
            ...
            <div css={versionsType !== 'modpacks' ? tw`hidden` : tw`flex grid grid-cols-2 col-span-2`}>
                <p>Select the platform you want to search modpacks.</p>
                <Select
                    onChange={(e) => {
                        setModpacktype(e.target.value);
                        setPage(1);
                    }}
        ...
        <ServerContentBlock title={'Minecraft Version ' + versionsType}>
            ...
            {mcversion !== null && (
                <p css={tw`text-lg text-center mb-2`}>
                    Current version:{' '}
                    <span css={tw`text-cyan-600`}>{mcversion.charAt(0).toUpperCase() + mcversion.slice(1)}</span>
                </p>
            )}
